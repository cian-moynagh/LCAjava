...
public class LCADAG {
    ...
    private final ArrayList<Integer>[] adjArray;
	  private final ArrayList<Integer>[] reverseAdjArray;

    //constructor
    public LCADAG(int Vrt)
	  {
		    this.Vrt = Vrt;
		    adjArray = (ArrayList<Integer>[]) new ArrayList[Vrt];

		    // For LCA
		    reverseAdjArray = (ArrayList<Integer>[]) new ArrayList[Vrt];
		    ...
		    for (int v = 0; v < Vrt; v++)
		    {
			       ...
			       //For LCA
			      reveseAdjArray[v] = new ArrayList<Integer>();
		    }
     }
     //Adds an edge from vertex v to vertex w if conditions met, returns true if edge was successfully added
     	public boolean addEdge(int v, int w)
     	{
     		...
     		if(v >= this.Vrt || w >= this.Vrt || v < 0 || w < 0)
     		{
     			return false;
     		}
     	...
     	public ArrayList<Integer> lowestCommonAncestor(int x, int y)
     	{
     		...
     		if(x==y || x>=this.Vrt || y>=this.Vrt || x<0 || y<0) { return lcas; } //If invalid input return empty bag.

     		DirectedDFS dfsObject = new DirectedDFS(this, x);
     		dfsObject.reverseDfs(this, x);
     		int xDist, yDist;

     		for(int v = 0; v < this.Vrt; v++)
     		{
     	...
     	private int getDistance(int x, int target)
     	{
     			...
     			if( x == target) { return 0; }
     		        ...
     		        int[] distTo = new int[this.Vrt];
     		        boolean[] marked = new boolean[this.Vrt];
     	...
     	private class DirectedDFS
     	{
     		...
     		{
     			marked = new boolean[G.Vrt()];
     			revMarked = new boolean[G.Vrt()];
     			dfs(G, s);
     		}
